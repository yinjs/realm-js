...
class ObjectWrap {
  ...
  private:
	...
	static const Napi::FunctionReference& get_or_create_realm_object_factory(Napi::Env env, Napi::Function& constructor, const realm::ObjectSchema& schema, const SharedRealm& realm);
	...
	static std::vector<Napi::PropertyDescriptor> create_napi_property_descriptors(Napi::Env env, const Napi::Object& constructorPrototype, const realm::ObjectSchema& schema, Napi::Array& ownKeys, bool redefine = true);
};
...
template<typename ClassType>
inline std::vector<Napi::PropertyDescriptor> ObjectWrap<ClassType>::create_napi_property_descriptors(Napi::Env env, const Napi::Object& constructorPrototype, const realm::ObjectSchema& schema, Napi::Array& ownKeys, bool redefine) {
	std::vector<Napi::PropertyDescriptor> properties;
	uint32_t index = 0;
...
template<typename ClassType>
const Napi::FunctionReference& ObjectWrap<ClassType>::get_or_create_realm_object_factory(Napi::Env env, Napi::Function& constructor, const realm::ObjectSchema& schema, const SharedRealm& realm) {
	...
	if (constructor.IsEmpty()) {
		...
		schemaObjectConstructor = env.Global().Get("Function").As<Napi::Function>()
			.New({
		...
		std::vector<Napi::PropertyDescriptor> properties = create_napi_property_descriptors(env, childPrototypeProperty, schema, ownKeys, true /*redefine*/);
		...
		//get all properties from the schema 
		auto ownKeys = Napi::Array::New(env);
		std::vector<Napi::PropertyDescriptor> properties = create_napi_property_descriptors(env, constructorPrototype, schema, ownKeys, false /*redefine*/);
	...
	schemaObject.factory = Persistent(env.Global().Get("Function").As<Napi::Function>()
		.New({
			// Arguments
			Napi::String::New(env, "ownKeysList"), 
